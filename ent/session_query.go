...
type SessionQuery struct {
	...
	withOwner     *UserQuery
	withApnsToken *APNsTokenQuery
	withFcmToken  *FCMTokenQuery
	withFKs       bool
...
// QueryApnsToken chains the current query on the "apns_token" edge.
func (sq *SessionQuery) QueryApnsToken() *APNsTokenQuery {
	query := (&APNsTokenClient{config: sq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := sq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
// QueryFcmToken chains the current query on the "fcm_token" edge.
func (sq *SessionQuery) QueryFcmToken() *FCMTokenQuery {
	query := (&FCMTokenClient{config: sq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := sq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (sq *SessionQuery) Clone() *SessionQuery {
	...
	return &SessionQuery{
		...
		withApnsToken: sq.withApnsToken.Clone(),
		withFcmToken:  sq.withFcmToken.Clone(),
...
// WithApnsToken tells the query-builder to eager-load the nodes that are connected to
// the "apns_token" edge. The optional arguments are used to configure the query builder of the edge.
func (sq *SessionQuery) WithApnsToken(opts ...func(*APNsTokenQuery)) *SessionQuery {
	query := (&APNsTokenClient{config: sq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	sq.withApnsToken = query
	return sq
}

// WithFcmToken tells the query-builder to eager-load the nodes that are connected to
// the "fcm_token" edge. The optional arguments are used to configure the query builder of the edge.
func (sq *SessionQuery) WithFcmToken(opts ...func(*FCMTokenQuery)) *SessionQuery {
	query := (&FCMTokenClient{config: sq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	sq.withFcmToken = query
	return sq
}
...
func (sq *SessionQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Session, error) {
	...
	if query := sq.withApnsToken; query != nil {
		if err := sq.loadApnsToken(ctx, query, nodes, nil,
			func(n *Session, e *APNsToken) { n.Edges.ApnsToken = e }); err != nil {
			return nil, err
		}
	}
	if query := sq.withFcmToken; query != nil {
		if err := sq.loadFcmToken(ctx, query, nodes, nil,
			func(n *Session, e *FCMToken) { n.Edges.FcmToken = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (sq *SessionQuery) loadApnsToken(ctx context.Context, query *APNsTokenQuery, nodes []*Session, init func(*Session), assign func(*Session, *APNsToken)) error {
	...
	query.withFKs = true
	query.Where(predicate.APNsToken(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(session.ApnsTokenColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.session_apns_token
		if fk == nil {
			return fmt.Errorf(`foreign-key "session_apns_token" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "session_apns_token" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
}
func (sq *SessionQuery) loadFcmToken(ctx context.Context, query *FCMTokenQuery, nodes []*Session, init func(*Session), assign func(*Session, *FCMToken)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uuid.UUID]*Session)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.Where(predicate.FCMToken(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(session.FcmTokenColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.session_fcm_token
		if fk == nil {
			return fmt.Errorf(`foreign-key "session_fcm_token" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "session_fcm_token" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
