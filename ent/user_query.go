...
func (uq *UserQuery) QueryOwnedGroups() *GroupQuery {
	query := (&GroupClient{config: uq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := uq.prepareQuery(ctx); err != nil {
			return nil, err
...
func (uq *UserQuery) QueryStudyLogs() *StudyLogQuery {
	query := (&StudyLogClient{config: uq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := uq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (uq *UserQuery) WithOwnedGroups(opts ...func(*GroupQuery)) *UserQuery {
	query := (&GroupClient{config: uq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	uq.withOwnedGroups = query
	return uq
}
...
func (uq *UserQuery) WithStudyLogs(opts ...func(*StudyLogQuery)) *UserQuery {
	query := (&StudyLogClient{config: uq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (uq *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := uq.withOwnedGroups; query != nil {
		if err := uq.loadOwnedGroups(ctx, query, nodes,
			func(n *User) { n.Edges.OwnedGroups = []*Group{} },
			func(n *User, e *Group) { n.Edges.OwnedGroups = append(n.Edges.OwnedGroups, e) }); err != nil {
			return nil, err
		}
	}
	if query := uq.withStudyLogs; query != nil {
...
func (uq *UserQuery) loadJoinedGroups(ctx context.Context, query *GroupQuery, nodes []*User, init func(*User), assign func(*User, *Group)) error {
	...
	query.Where(func(s *sql.Selector) {
		...
		s.Join(joinT).On(s.C(group.FieldID), joinT.C(user.JoinedGroupsPrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(user.JoinedGroupsPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(user.JoinedGroupsPrimaryKey[0]))
...
func (uq *UserQuery) loadOwnedGroups(ctx context.Context, query *GroupQuery, nodes []*User, init func(*User), assign func(*User, *Group)) error {
	...
	for _, n := range neighbors {
		fk := n.user_owned_groups
		if fk == nil {
			return fmt.Errorf(`foreign-key "user_owned_groups" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "user_owned_groups" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
}
func (uq *UserQuery) loadStudyLogs(ctx context.Context, query *StudyLogQuery, nodes []*User, init func(*User), assign func(*User, *StudyLog)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uuid.UUID]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.Where(predicate.StudyLog(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.StudyLogsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.user_study_logs
		if fk == nil {
			return fmt.Errorf(`foreign-key "user_study_logs" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
