...
// QuerySharedStudyLogs chains the current query on the "shared_study_logs" edge.
func (gq *GroupQuery) QuerySharedStudyLogs() *StudyLogQuery {
	query := (&StudyLogClient{config: gq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := gq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (gq *GroupQuery) QueryInviteCodes() *InviteCodeQuery {
	query := (&InviteCodeClient{config: gq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := gq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
// WithSharedStudyLogs tells the query-builder to eager-load the nodes that are connected to
// the "shared_study_logs" edge. The optional arguments are used to configure the query builder of the edge.
func (gq *GroupQuery) WithSharedStudyLogs(opts ...func(*StudyLogQuery)) *GroupQuery {
	query := (&StudyLogClient{config: gq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (gq *GroupQuery) WithInviteCodes(opts ...func(*InviteCodeQuery)) *GroupQuery {
	query := (&InviteCodeClient{config: gq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	gq.withInviteCodes = query
	return gq
}
...
func (gq *GroupQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Group, error) {
	...
	if query := gq.withSharedStudyLogs; query != nil {
	...
	if query := gq.withInviteCodes; query != nil {
		if err := gq.loadInviteCodes(ctx, query, nodes,
			func(n *Group) { n.Edges.InviteCodes = []*InviteCode{} },
			func(n *Group, e *InviteCode) { n.Edges.InviteCodes = append(n.Edges.InviteCodes, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (gq *GroupQuery) loadMembers(ctx context.Context, query *UserQuery, nodes []*Group, init func(*Group), assign func(*Group, *User)) error {
	...
	query.Where(func(s *sql.Selector) {
		...
		s.Join(joinT).On(s.C(user.FieldID), joinT.C(group.MembersPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(group.MembersPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(group.MembersPrimaryKey[1]))
...
func (gq *GroupQuery) loadOwner(ctx context.Context, query *UserQuery, nodes []*Group, init func(*Group), assign func(*Group, *User)) error {
	...
	for i := range nodes {
		if nodes[i].user_owned_groups == nil {
			continue
		}
		fk := *nodes[i].user_owned_groups
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "user_owned_groups" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (gq *GroupQuery) loadSharedStudyLogs(ctx context.Context, query *StudyLogQuery, nodes []*Group, init func(*Group), assign func(*Group, *StudyLog)) error {
	edgeIDs := make([]driver.Value, len(nodes))
	byID := make(map[uuid.UUID]*Group)
	nids := make(map[uuid.UUID]map[*Group]struct{})
	for i, node := range nodes {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(group.SharedStudyLogsTable)
		s.Join(joinT).On(s.C(studylog.FieldID), joinT.C(group.SharedStudyLogsPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(group.SharedStudyLogsPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(group.SharedStudyLogsPrimaryKey[1]))
		s.AppendSelect(columns...)
		s.SetDistinct(false)
	})
	if err := query.prepareQuery(ctx); err != nil {
	...
	neighbors, err := withInterceptors[[]*StudyLog](ctx, query, qr, query.inters)
	if err != nil {
		return err
	}
	for _, n := range neighbors {
...
func (gq *GroupQuery) loadInviteCodes(ctx context.Context, query *InviteCodeQuery, nodes []*Group, init func(*Group), assign func(*Group, *InviteCode)) error {
	...
	query.Where(predicate.InviteCode(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(group.InviteCodesColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.group_invite_codes
		if fk == nil {
			return fmt.Errorf(`foreign-key "group_invite_codes" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "group_invite_codes" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
