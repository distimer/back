...
func (sq *SubjectQuery) QueryCategory() *CategoryQuery {
...
func (sq *SubjectQuery) QueryStudyLogs() *StudyLogQuery {
	query := (&StudyLogClient{config: sq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := sq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (sq *SubjectQuery) WithCategory(opts ...func(*CategoryQuery)) *SubjectQuery {
	query := (&CategoryClient{config: sq.config}).Query()
...
func (sq *SubjectQuery) WithStudyLogs(opts ...func(*StudyLogQuery)) *SubjectQuery {
	query := (&StudyLogClient{config: sq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (sq *SubjectQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Subject, error) {
	...
	if query := sq.withCategory; query != nil {
		if err := sq.loadCategory(ctx, query, nodes, nil,
			func(n *Subject, e *Category) { n.Edges.Category = e }); err != nil {
			return nil, err
		}
	}
	if query := sq.withStudyLogs; query != nil {
...
func (sq *SubjectQuery) loadCategory(ctx context.Context, query *CategoryQuery, nodes []*Subject, init func(*Subject), assign func(*Subject, *Category)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*Subject)
	for i := range nodes {
		if nodes[i].category_subjects == nil {
			continue
		}
		fk := *nodes[i].category_subjects
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "category_subjects" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (sq *SubjectQuery) loadStudyLogs(ctx context.Context, query *StudyLogQuery, nodes []*Subject, init func(*Subject), assign func(*Subject, *StudyLog)) error {
	...
	query.Where(predicate.StudyLog(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(subject.StudyLogsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.subject_study_logs
		if fk == nil {
			return fmt.Errorf(`foreign-key "subject_study_logs" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
